<?php
/*
Plugin Name: ML Post Slider
Plugin URI: http://www.mlipinski.pl/plugins/ml-post-slider
...
class ml_post_slider_widget extends WP_Widget{
	...
	function form($instance){
		$defaults= array( 'ml_category' => '', 'ml_width' => 100, 'ml_height'=> 100, 'ml_dimension' => 'px', 'ml_delay' => 1000, 'ml_unique_id' => date('Ymdhis'), 'ml_effect' => 'fade');
		$instance = wp_parse_args( (array) $instance, $defaults);
		$ml_category = $instance['ml_category'];
		$ml_width= $instance['ml_width'];
		$ml_height= $instance['ml_height'];
		$ml_dimension= $instance['ml_dimension'];
		...
		<p>Slider width:
			<input type="text" name="<?php echo $this->get_field_name('ml_width'); ?>" value="<?php echo esc_attr( $ml_width); ?>" size="3" /> <i class="ml-post-slider-i">[<?php echo esc_attr( $ml_dimension) ?>]</i>
		</p>
		<p>Slider height:
			<input type="text" name="<?php echo $this->get_field_name('ml_height'); ?>" value="<?php echo esc_attr( $ml_height); ?>" size="3" /> <i>[px]</i>
		</p>
		<p>Width dimensioning:
			<input type="radio" class="ml-post-slider-radio" name="<?php echo $this->get_field_name('ml_dimension'); ?>" value="px" <?php if (esc_attr( $ml_dimension)==='px') echo 'checked="checked"'?> />[px]
			<input type="radio" class="ml-post-slider-radio" name="<?php echo $this->get_field_name('ml_dimension'); ?>" value="%" <?php if (esc_attr( $ml_dimension)==='%') echo 'checked="checked"'?> />[%]
		</p>
		<script>
			jQuery(".ml-post-slider-radio").click(function(){
				jQuery('.ml-post-slider-i').text("["+jQuery(this).val()+"]");
			});
		</script>
		<p>Delay:
	...
	function update($new_instance, $old_instance){
		...
		if ((strlen(sanitize_text_field($new_instance['ml_dimension']))<255) && ($new_instance['ml_dimension']=="%")) $instance['ml_dimension'] = "%";
		else $instance['ml_dimension'] = "px";
...
// Here is the most important function. This function displays Slider. If you want change something, please do it here;)
	function widget($args, $instance){
		...
		echo '<div class="parrent_ml_post_slider'.$instance['ml_unique_id'].'" style="position:relative; display:block;width:'.($instance['ml_width']).$instance['ml_dimension'].'; height:'.($instance['ml_height']).'px;">';
		// This div contains all slides without navigation arrows
		echo '<div class="ml_post_slider'.$instance['ml_unique_id'].'" style="width:100; overflow:hidden;background-colour:transparent;position:relative;">'; 
		foreach($posts_array as $post){
			echo '<div style="width:100%; height:'.$instance['ml_height'].'px; overflow:hidden;">'.do_shortcode($post->post_content).'</div>';
		}
